<script lang="ts">
    ...
    function calcDept(capital: number, installment: number, interest: number): DeptCalculation {
		var cost = 0;
		var iterations = 0;
		var items = [];
		while (capital > 0) {
		...
		return { cost: Math.round(cost), months: iterations, items };
	}

    function calculate() {
        const standardResult = calcDept(captial, installment, interest);
        const surchargeResult = calcDept(captial, installment + surcharge, interest);
...
<div>
    ...
    <form on:submit={calculate}>
        ...
        <div>
            <label for="interest" class="form-label">Kwota miesięcznej raty</label>
            <input 
                type="number" 
                class="form-control" 
                step="0.01" 
                ...
                on:keydown={() => reset()}
                required
                placeholder="np 2459,01 zł"
            />
        </div>

        <div>
            <label for="interest" class="form-label">Kwota miesięcznej nadpłaty</label>
            <input 
                type="number" 
                class="form-control" 
                step="0.01" 
